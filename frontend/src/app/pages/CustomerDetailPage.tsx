...
const transactionLabels: Record<TimeTransactionType, string> = {
  PURCHASE: "Compra",
...
export function CustomerDetailPage() {
  ...
  const [wallet, setWallet] = useState<TimeWallet | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadCustomer() {
      ...
      try {
        const [customerData, walletData] = await Promise.all([
  ...
  useEffect(() => {
    ...
    async function loadTransactions() {
      ...
      try {
        const data = await listCustomerTransactions(
          customerId!,
          PAGE_SIZE,
          offset,
        );
        ...
        if (!cancelled) {
          setError("No fue posible cargar el historial de tiempo.");
        }
      } finally {
        if (!cancelled) {
  ...
  if (isLoadingCustomer) {
    return <section className="content-state">Cargando cliente...</section>;
  }
    ...
    <section className="customer-detail-page">
      ...
      <header className="customer-detail-header">
        <div>
          ...
          <p className="page-description">@{customer.username}</p>
        </div>

        <span
          className={
            customer.is_active ? "status-badge active" : "status-badge inactive"
          }
        >
      ...
      <div className="customer-summary-grid">
        <article className="summary-card">
          <span className="summary-label">Tiempo disponible</span>

          <strong>{formatDuration(wallet.available_seconds)}</strong>
        </article>

        <article className="summary-card">
          <span className="summary-label">Tiempo reservado</span>

          <strong>{formatDuration(wallet.reserved_seconds)}</strong>
        </article>

        <article className="summary-card">
          <span className="summary-label">Registrado</span>
      ...
      <section className="history-section">
        ...
        {isLoadingTransactions ? (
          <div className="content-state">Cargando movimientos...</div>
              ...
              <tbody>
                {transactions.map((transaction) => (
                  <tr key={transaction.id}>
                    ...
                    <td>{transactionLabels[transaction.transaction_type]}</td>
                    ...
                    <td>
                      {formatSignedDuration(transaction.reserved_seconds_delta)}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="pagination">
          <button
            ...
            disabled={offset === 0 || isLoadingTransactions}
          ...
          <button
            type="button"
            className="secondary-button"
            disabled={transactions.length < PAGE_SIZE || isLoadingTransactions}
